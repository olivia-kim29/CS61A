...
(define (longest-increasing-subsequence lst)
    ...
    (if (null? lst)
        ...
        (begin
            ...
            (define rest (cdr lst))
            (define large-values-rest (larger-values first rest))
            ...
            (if (or (null? rest) (> (car rest) first))
              with-first
              without-first
            )
        )
...
(define (derive-exp exp var)
        (make-product (make-product (exponent exp) (make-exp (base exp) (- (exponent exp) 1)))
              (derive (base exp) var)
        )
)
