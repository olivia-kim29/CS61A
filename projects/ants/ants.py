    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 9
                if self.ant.is_container == True and self.ant.can_contain(insect) == True:
                    self.ant.contained_ant = insect
                elif self.ant.is_container == False and insect.can_contain(self.ant) == True:
                    insect.place = self
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        if self.place.ant is not None:
            return self.place.ant.blocks_path == True
        elif self.place.ant is None:
            return False
        # END Problem 7
    ...
    def nearest_bee(self, hive):
        ...
        if self.min_range > 0:
            for i in range(self.min_range):
                ...
                counter += 1
        while check_place.bees == []:
            if check_place.entrance == hive:
                return None
            counter += 1
            check_place = check_place.entrance
        if self.max_range < counter:
            return None
        if self.min_range > counter:
            return None
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 5
        fire_ant_past_place = self.place
        copy_of_bees = fire_ant_past_place.bees[:]
        Insect.reduce_armor(self, amount)
        if self.place == None:
            for i in range(len(fire_ant_past_place.bees)):
                Insect.reduce_armor(copy_of_bees[i], self.damage)
    ...
    def action(self, colony):
        ...
        if self.place.bees != []:
            for i in range(len(self.place.bees)):
                Insect.reduce_armor(copy_bees[i], self.damage)
        # END Problem 7
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.true_queen == False:
            ...
            check_place = self.place
            while check_place.exit != None:
                check_place = check_place.exit
                if check_place.ant != None and check_place.ant.is_buffed == False:
                    check_place.ant.damage *= 2
                    check_place.ant.is_buffed = True
                if check_place.ant != None and check_place.ant.is_container == True and check_place.ant.contained_ant != None and check_place.ant.contained_ant.is_buffed == False:
                    check_place.ant.contained_ant.damage *= 2
                    check_place.ant.contained_ant.is_buffed = True
...
def make_slow(action, bee):
    ...
    s an effect that takes an action method and a bee, and returns a new action method
    that performs the original action on turns where colony.time is even and does nothing on other turns.
    """
    # BEGIN Problem EC
