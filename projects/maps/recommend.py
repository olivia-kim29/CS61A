...
def group_by_centroid(restaurants, centroids):
    ...
    """
    # BEGIN Question 4
    closest_centroid = lambda r: find_closest(restaurant_location(r), centroids)
    locations = [[closest_centroid(r),r] for r in restaurants]
    return group_by_first(locations)

def find_centroid(cluster):
    """Return the centroid of the locations of the restaurants in cluster."""
    # BEGIN Question 5
    centroid_latitude = mean([restaurant_location(n)[0] for n in cluster])
    centroid_longitude = mean([restaurant_location(n)[1] for n in cluster])
...
def find_predictor(user, restaurants, feature_fn):
    ...
    S_xy = sum([r[0]*r[1] for r in zip([(x - mean(xs)) for x in xs], [(y - mean(ys)) for y in ys])])
...
def rate_all(user, restaurants, feature_fns):
    ...
    for review in restaurants:
        name = restaurant_name(review)
        if review in reviewed:
            ratings[name] = user_rating(user, name)
        else:
            ratings[name] = predictor(review)
    return ratings
    # END Question 9
