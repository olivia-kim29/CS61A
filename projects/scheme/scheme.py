...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('length does not match')
        child = Frame(self)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        first, formals = target.first, target.second
        body = expressions.second
        env.define(first, LambdaProcedure(formals, body, env))
        return first
        # END PROBLEM 10
    else:
